$(document).ready(function(){
	checkGmailSetup();
	//checkNursingSurveySetup();
	checkSurveySetup();
});


function checkSurveySetup(){
	var sessionid = getCookie("MoodleSession");
        $.get('/wosc/dosurvey.php', { session: sessionid} )
                .done(function(data) {
                        var obj = jQuery.parseJSON(data);
                        if (obj.nursingSurvey != "0"){
                                var studentid = obj.studentNumber;
                                var termcode = obj.nursingSurvey;
                                //alert("StudentId = " + studentid + "\nTermcode = " + termcode);
                                $("#surveydestination").attr("href","https://census.wosc.edu/index.php/survey/index/sid/388841/newtest/Y/lang/en/388841X117X554/" + studentid + "/388841X117X579/" + termcode);
                                $("#surveydestinationx").attr("href","https://census.wosc.edu/index.php/survey/index/sid/388841/newtest/Y/lang/en/388841X117X554/" + studentid + "/388841X117X579/" + termcode);
				$('#noticeContent').html("As a Western Oklahoma State College Nursing Student, you must complete this <strong>MANDITORY</strong> survey.");
				$('#noticeHeader').html("MANDITORY Nursing Survey");
                                $("#woscnotice").css("display", "block");
				
                        }
			else if (obj.studentSurvey != "0"){
                                	var studentid = obj.studentNumber;
                                	var termcode = obj.studentSurvey;
                                	//alert("StudentId = " + studentid + "\nTermcode = " + termcode);
                                	$("#surveydestination").attr("href","https://census.wosc.edu/index.php/survey/index/sid/749162/newtest/Y/lang/en/749162X92X317/" + studentid + "/749162X92X318/" + termcode);
                                	$("#surveydestinationx").attr("href","https://census.wosc.edu/index.php/survey/index/sid/749162/newtest/Y/lang/en/749162X92X317/" + studentid + "/749162X92X318/" + termcode);
                                	$("#woscnotice").css("display", "block");
                        	}
			});
         
}

/* function checkSurveySetup(){
	var sessionid = getCookie("MoodleSession");
	$.get('/wosc/dosurvey.php', { session: sessionid} )
		.done(function(data) {
			var obj = jQuery.parseJSON(data);
			if (obj.studentSurvey != "0"){
				var studentid = obj.studentid;
				var termcode = obj.studentSurvey;
				//alert("StudentId = " + studentid + "\nTermcode = " + termcode);
				$("#surveydestination").attr("href","https://census.wosc.edu/index.php/survey/index/sid/749162/newtest/Y/lang/en/749162X92X317/" + studentid + "/749162X92X318/" + termcode);
				$("#surveydestinationx").attr("href","https://census.wosc.edu/index.php/survey/index/sid/749162/newtest/Y/lang/en/749162X92X317/" + studentid + "/749162X92X318/" + termcode);
				$("#woscnotice").css("display", "block");
			}
		});
}
*/

function checkGmailSetup(){

        var sessionid = getCookie('MoodleSession');
        $.ajax({
                url:"/wosc/checkgmail.php",
                data: {'MoodleSession': sessionid },
                success: function(data){
		       //alert(data);
                       processGmailResponse(data);
                },
                type: 'GET'
        });
}

function authorizeMoodle(){
        window.location.href = 'https://moodle.wosc.edu/wosc/checkgmail.php?action=getToken';
}

function processGmailResponse(data){

        if(data == "0"){
                var html = '<p style="font-weight:bold"> You have not authorized Moodle to access your email yet. <button type="button" id="authorize" onClick="authorizeMoodle()">Authorize</button></p>';
		//alert(html);
                $('#gmailStatus').html(html);
                $("#gmailStatus").css("display", "block");
        }
        else {
			if(data != "null"){
                $('#gmailStatus').html(data);
                $("#gmailStatus").css("display", "none");
		$(".avatar > .userpicture").addClass('bell');
		$(".avatar").addClass('displaybadge');
		$(".avatar > .userpicture").attr('src', 'data:image/png;base64,');
			}
        }
}


function getCookie(cname) {
    var name = cname + "=";
    var ca = document.cookie.split(';');
    for(var i=0; i<ca.length; i++) {
        var c = ca[i];
        while (c.charAt(0)==' ') c = c.substring(1);
        if (c.indexOf(name) == 0) return c.substring(name.length,c.length);
    }
    return "";
}
